...
const layananPublik = async (req, res, next) => {
  try {
    ...
    if (jenis_layanan_id&&jenis_layanan_id!=="") {
      where.kategori_layanan_publik = {
        jenis_layanan: {
          id: parseInt(jenis_layanan_id),
        },
      };
    }
    ...
    if (status&&status!=="") {
      where.status_riwayat_terbaru = status;
    }


    if (kategori_id&&kategori_id!=="") {
      where.fk_kategori_layanan_publik_id = parseInt(kategori_id);
    }

    if (tanggal&&tanggal!=="") {
      where.tanggal_surat = new Date(tanggal);
    }
...
const filter = async (req, res, next) => {
  try {
    ...
    if (
      status &&
      ["Menunggu", "Ditolak", "Dikerjakan", "Selesai"].includes(status)
    ) {
    ...
    if (kategori) {
      ...
      if (kategoriLayanan) {
        ...
        return res
          .status(404)
          .json({ message: "Kategori layanan publik not found." });
      }
    }
    ...
    if (jenisLayananPublik) {
      ...
      if (jenisLayanan) {
        ...
        filters.fk_kategori_layanan_publik_id = {
          in: kategoriIds.map((k) => k.id),
        };
      } else {
        return res
          .status(400)
          .json({ message: "Invalid jenis layanan publik." });
      }
    }
    ...
    res
      .status(200)
      .json({ message: "Successfully.", count: total, data: data });
  } catch (error) {
    console.error("ERROR =>", error);
    next(new ResponseError(500, "Internal server error"));
  }
...
const tambahLayananPublik = async (req, res, next) => {
  try {
    ...
    const daerah = await prisma.daerah.findFirst({
      where: { id: pengguna.penduduk.fk_daerah_id },
      select: { id: true },
    });
    ...
    if (!kategoriLayanan) {
      return res
        .status(404)
        .json({ message: "Kategori layanan publik is not found." });
    }

    const newLayananPublik = await prisma.layananPublik.create({
      data: {
        fk_kategori_layanan_publik_id: parseInt(
          data.fk_kategori_layanan_publik_id
        ),
        diajukan_oleh_id: userId,
...
const statutDitolakLayananPublik = async (req, res, next) => {
  try {
    ...
    const statusDitolak = {
      status: "Ditolak",
      waktu: new Date().toISOString(),
      alasan: dataRequest.alasan || null,
    };
