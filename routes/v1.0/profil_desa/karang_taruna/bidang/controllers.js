...
const getBidangById = async (req, res, next) => {
    try {
        ...
        const bidang = karangTaruna.data_bidang.find(bidang => bidang.id === bidangId)
...
const createBidang = async (req, res, next) => {
    try {
        ...
        const bidangExists = karangTaruna.data_bidang.some(existingBidang => existingBidang.bidang === bidang)
...
const updateBidang = async (req, res, next) => {
    try {
        ...
        const updatedBidang = karangTaruna.data_bidang.find(bidang => bidang.id === bidangId)
...
const removeBidang = async (req, res, next) => {
    try {
        ...
        const index = karangTaruna.data_bidang.findIndex(bidang => bidang.id === bidangId)

        if (index === -1) {
            return res.status(400).json({ message: "Bidang is not found" });
        }

        const updatedDataBidang = [...karangTaruna.data_bidang.slice(0, index), ...karangTaruna.data_bidang.slice(index + 1)]
...
const removeAnggotaBidang = async (req, res, next) => {
    try {
        ...
        const bidangIndex = karangTaruna.data_bidang.findIndex(bidang => bidang.id === bidangId)

        if (bidangIndex === -1) {
            return res.status(400).json({ message: "Bidang not found" });
        }

        const bidang = karangTaruna.data_bidang[bidangIndex]
        ...
        const updatedDataBidang = [...karangTaruna.data_bidang];
